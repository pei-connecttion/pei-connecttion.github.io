import { useEffect, useState } from 'react';
import MarkdownIt from 'markdown-it';
// static fallback import (raw) — Vite supports ?raw to import file content as string
import calendarRaw from '/src/content/calendar/calendar.md?raw';

const md = new MarkdownIt({ html: true });

export default function PricingHero(block) {
  // initialize with static fallback so page shows content even if glob fails
  const [calendarMarkdown, setCalendarMarkdown] = useState(typeof calendarRaw === 'string' ? calendarRaw : '');

  useEffect(() => {
    const modules = import.meta.glob('/src/content/calendar/*.md', { as: 'raw' });
    const load = async () => {
      const keys = Object.keys(modules);
      if (keys.length === 0) {
        // leave the static fallback in place
        return;
      }
      // pick a file: prefer block.md_file if specified
      let chosen = keys[0];
      if (block && block.md_file) {
        const match = keys.find(k => k.endsWith('/' + block.md_file));
        if (match) chosen = match;
      }
      try {
        if (modules[chosen]) {
          const loaded = await modules[chosen]();
          const mdText = typeof loaded === 'string' ? loaded : (loaded && loaded.default) ? loaded.default : '';
          if (mdText && mdText.length > 0) setCalendarMarkdown(mdText);
        }
      } catch (e) {
        console.error('Failed loading calendar md', e);
        // keep static fallback
      }
    };
    load();
  }, [block]);

  return (
    <section className="pricing-inner @@bg-color">
      <div className="container">
        <div className="row">
          <div className="col-lg-8 mx-auto">
            <div className="section-header position-relative text-center">
              <h2>{block.title}</h2>
              <div
                dangerouslySetInnerHTML={{
                  __html: md.render(block.description),
                }}
              />
            </div>
          </div>
        </div>
        {/* calendar content rendered from markdown file as a styled table */}
        <div className="row">
          <div className="col-12">
            <style>{`
              .pricing-calendar { padding-top: 32px; padding-bottom: 32px; }
              .pricing-calendar__table { width: 100%; border-collapse: collapse; }
              .pricing-calendar__table thead th { text-align: left; font-weight: 800; padding: 18px 12px; font-size: 16px; }
              .pricing-calendar__table tbody tr { border-top: 1px solid #e6e6e6; }
              .pricing-calendar__table td { vertical-align: top; padding: 24px 12px; }
              .pricing-calendar__milestone a { font-weight: 800; color: #0f1720; text-decoration: underline; }
              .pricing-calendar__date { color: #6b7280; white-space: nowrap; width: 180px; }
              .pricing-calendar__tasks { color: #6b7280; }
              .pricing-calendar__tasks ul { margin: 0; padding-left: 18px; }
              @media (max-width: 768px) {
                .pricing-calendar__date { display: none; }
                .pricing-calendar__table td { padding: 12px 8px; }
              }
            `}</style>
            {calendarMarkdown ? (
              (() => {
                const parseCalendar = (mdText) => {
                  const lines = mdText.split(/\r?\n/);
                  const entries = [];
                  let current = null;
                  for (let rawLine of lines) {
                    const line = rawLine.trim();
                    if (!line) continue;
                    const headingMatch = line.match(/^#{3,4}\s*(.+)/);
                    if (headingMatch) {
                      const text = headingMatch[1].trim();
                      // prefer em-dash separator
                      let title = '';
                      let date = '';
                      if (text.indexOf('—') !== -1) {
                        const parts = text.split(/\s+—\s+/);
                        title = parts[0].trim();
                        date = parts.slice(1).join(' — ').trim();
                      } else if (/\w/.test(text) && text.indexOf(' - ') !== -1 && /[A-Za-zÀ-ÿ]/.test(text)) {
                        const parts = text.split(/\s+-\s+/);
                        title = parts[0].trim();
                        date = parts.slice(1).join(' - ').trim();
                      } else {
                        // treat as date-only heading
                        title = '';
                        date = text;
                      }
                      current = { title, date, tasks: [] };
                      entries.push(current);
                    } else if (line.startsWith('- ')) {
                      if (!current) { current = { title: '', date: '', tasks: [] }; entries.push(current); }
                      current.tasks.push(line.replace(/^-\s+/, '').trim());
                    }
                  }
                  return entries;
                };

                const entries = parseCalendar(calendarMarkdown);
                return (
                  <div className="pricing-calendar">
                    <table className="pricing-calendar__table">
                      <thead>
                        <tr>
                          <th>Milestone</th>
                          <th>Date (2025)</th>
                          <th>Task List</th>
                        </tr>
                      </thead>
                      <tbody>
                        {entries.map((e, i) => (
                          <tr key={i}>
                            <td className="pricing-calendar__milestone">{e.title ? <a href="#">{e.title}</a> : ''}</td>
                            <td className="pricing-calendar__date">{e.date}</td>
                            <td className="pricing-calendar__tasks">
                              <ul>
                                {e.tasks.map((t, idx) => (<li key={idx}>{t}</li>))}
                              </ul>
                            </td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                  </div>
                );
              })()
            ) : (
              <div className="pricing-calendar__placeholder">No calendar content found.</div>
            )}
          </div>
        </div>
      </div>
    </section>
  );
}
